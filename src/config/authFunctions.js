...
import commonQueries from '../models/queries/commonQueries.js';
...
export async function checkAuthorization(userID, roleID) {
    try {
        const [result] = await promisePool.query(commonQueries.checkAuthorization, [userID, roleID]);
...
export async function getUserRole(userID) {
    try {
        const [result] = await promisePool.query(commonQueries.getUserRole, [userID]);
